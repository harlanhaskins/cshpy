...
class Webnews:
    ...
    def _post(self, target, params={}):
        url = self.prefix + target
        payload = dict(self.payload.items() + params.items())
        return requests.post(url, params=payload, headers=self.headers,
                             verify=False)

    def _put(self, target, params={}):
        url = self.prefix + target
        payload = dict(self.payload.items() + params.items())
        return requests.put(url, params=payload, headers=self.headers,
                            verify=False)

    def _delete(self, target, params={}):
        url = self.prefix + target
        payload = dict(self.payload.items() + params.items())
        return requests.delete(url, params=payload, headers=self.headers,
                               verify=False)
    ...
    def newsgroup_names(self):
        names = []
        group_dicts = self.newsgroups()[u'newsgroups']
        for groupDict in group_dicts:
    ...
    def get_newsgroup_index(self, newsgroup):
        return json.loads(
            self._get(newsgroup + '/index', {'newsgroup': newsgroup}).content)
